...
function LoginOnSuccess(result){
    ...
    // Установка обработчика нажатия кнопки для отправки сообщения
...
//каждые 5 секунд обновляем поле чата
function Refresh() {
    var href = "/Home?user=" + encodeURIComponent($("#Username").text());
    $("#ActionLink").attr("href", href).click();
    setTimeout("Refresh();", 5000);
...
// при успешном получении от сервера
